...
plot(alp(:,1),alp(:,2))
set(gca,'FontSize',13)
title('Coeficiente de Atenuacion','FontSize',18) ; xlabel('Longitud de Onda [nm]','FontSize',16) ; ylabel('Atenuación [dB/km]','FontSize',16) 
set( gcf,'PaperSize',[29.7 21.0], 'PaperPosition',[0 0 29.7 21.0])
% SAVE:
print -dpdf 'Cascada_Atenuacion' 
...
for span=1:Nspans
    ...
    % --- Calculo Siguiente EDFA --- %
    %EDFA = Span_EDFA_GEF_MMvPCCv3(Fibra,Signal,Pump,P0_ASE);
...
for span = 1:Nspans+1
    ...
    for s = 1:length(Signal.modos)
        ...
        title(strcat('Ganancias Por Span Modo ',modos(s)),'FontSize',18) ; legend('Location', 'southoutside','Orientation','horizontal','Box','off', "NumColumns" , 4,'FontSize',13); 
        xlabel('Longitud de onda [nm]','FontSize',16) ; ylabel(ylab,'FontSize',16)
        ylim([15 17.5])%ylim([16.5 18.5])
    end 
end ; clear s ejex leyenda;
set( gcf,'PaperSize',[29.7 21.0], 'PaperPosition',[0 0 29.7 21.0])
% SAVE:
% print -dpdf 'Cascada_Ganancias_LP01' % Cascada_Ganancias_LP11a
...
for span = 1:Nspans+1
    ...
    for s = 1:length(Signal.modos)
        ...
        title(strcat('Figuras de Ruido Por Span Modo ',modos(s)),'FontSize',18) ; 
        legend('Location', 'southoutside','Orientation','horizontal','Box','off', "NumColumns" , 4,'FontSize',13); 
        xlabel('Longitud de onda [nm]','FontSize',14) ; ylabel(ylab,'FontSize',14)
    end 
    %ax.ColorOrderIndex = 1;
end ; clear s ejex leyenda;
set( gcf,'PaperSize',[29.7 21.0], 'PaperPosition',[0 0 29.7 21.0])
% print -dpdf 'Cascada_NF01' % Cascada_NF11a
