function edfa = EDFA_MMvpi2_OSNRForward(fibra,signal,pump,ASE)
% Datos de entrada:
% signal (struct)
...
F_ase1=189.25e12; % (Ex DifPump:191.19421875e12)
F_ase_end=198.85e12; % (Ex DifPump: 193.64421875e12)
delta_noise=12.5e9;
...
if(isfield(fibra,'ASEFlag')) % retorna [0,0,0], evita calcular espectro ASE
    [lambda_ase,frequency_ase] = ase_freqVPI(0,0,0,c,1);
else
...
%                             v_s = c./lambda_s;
%                             Gamma_s = gamma_s.(ModoS(s)){i};
                            Pan.(ModoS(s))(i,Nz-z+1) = P_ase0;%+((N2(Nz-z+1,:)*sigma_ems(lambda_s(i))-N1(Nz-z+1,:)*sigma_abs(lambda_s(i)))*Gamma_s* P_ase0 + 2*N2(Nz-z+1,:)*sigma_ems(lambda_s(i))*Gamma_s*h*v_s(i)*d_vk)*(-1*del_z)/(1+P_ase0/Psat);
                        end
                    else
                        for i = 1:1:Nwl
                            lambda_s = signal.lambda.(ModoS(s));
...
%                     end
                ...
                parfor i = 1:Nch_ase
                    ...
                    AuxMatrix(i) = Pan_sp.(ModoS(s))(i,Nz-z+1+1) + (((N2(Nz-z+1))*sigma_ems(lambda_ase(i)) - (N1(Nz-z+1))*sigma_abs(lambda_ase(i)))*Gamma_s*Pan_sp.(ModoS(s))(i,Nz-z+1+1) + 2*(N2(Nz-z+1))*sigma_ems(lambda_ase(i))*Gamma_s*h*v_s_sp(i)*d_vk_sp)*del_z;%/(1+Pan_sp.(ModoS(s))(i,Nz-z+1+1)/Psat);
                end
                Pan_sp.(ModoS(s))(:,Nz-z+1) = AuxMatrix(:) ;
            end
        end
    ...
    NoiseBin_num=TotalNoise_bandwidth/d_vk + 1; % Number of Noise Bins
    ...
    for s=1:Smod
        ...
        for i=1:NoiseBin_num % Noise Bins power 
            if(i==1)
                ...
                NoiseBin_power.(ModoS(s))(i)=mean(Pap_spectrum(aux_f:aux_f+NoiseBin_aux));
                aux_f=aux_f+NoiseBin_aux;
            else % i==NoiseBin_num
                f_left(i)=frequency_ase(aux_f);
                f_right(i)=frequency_ase(aux_f+NoiseBin_aux/2);
                NoiseBin_power.(ModoS(s))(i)=mean(Pap_spectrum(aux_f:aux_f+NoiseBin_aux/2));
            end
            end
        end
    ...
    for s = 1:1:Smod
        ...
        for i = 1:1:Nwl
            ...
            OSNRForward.(ModoS(s))(i,:) = 10*log10(Psp.(ModoS(s))(i,:)/1e-3) - 10*log10(Pap_forsignal.(ModoS(s))(i,:)/1e-3);
    ...
    for s=1:Smod
        ...
        sdm.(ch).Pase.frequency_vector=frequency_ase;

        sdm.(ch).PaseF.(ModoS(s))=10*log10(Pap_sp.(ModoS(s))./1e-3);
        sdm.(ch).PaseB.(ModoS(s))=10*log10(Pan_sp.(ModoS(s))./1e-3);
        sdm.(ch).NoiseBins.((ModoS(s)))=[f_left' f_right' 10*log10(NoiseBin_power.(ModoS(s))./1e-3)']; 
