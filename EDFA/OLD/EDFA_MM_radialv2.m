function edfa = EDFA_MM_radialv2(fibra,signal,pump,ASE)
% Datos de entrada:                         
% signal (struct)                           
...
N = zeros(1,fibra.M-1);
parfor i = 1:fibra.M-1                     % Perfil de dopaje
...
for s = 1:1:length(signal.modos) % Mode overlap factor for signal entre modo y perfil de dopaje
    ...
    for i=1:1:Nwl
       ...
       CoupCoef = 0;      
        parfor p = 1:length(pump.modos) % Agregar coeficiente de acoplamiento entre bombeo y señal
            Nwlp = length(pump.lambda.(strcat("LP_",pump.modos(p))));
            for pp=1:1:Nwlp
                lambda_p = pump.lambda.(strcat("LP_",pump.modos(p)));
...
for n = 1:1:Sch     % Iteración en nucleos
    ...
    % Calculo de densidades de iones en estado basal y excitado (N1 y N2)
        sig_xx = zeros(1,fibra.M-1 );%0;
        sig_yy = zeros(1,fibra.M-1 );%0;                    % Inicialización de variables
        ase_xx = zeros(1,fibra.M-1 );%0;
        ase_yy = zeros(1,fibra.M-1 );%0;
        pmp_xx = zeros(1,fibra.M-1 );%0;
        pmp_yy = zeros(1,fibra.M-1 );%0;
                    ...
                    if(z == 1)
                        ...
                        for p = 1:1:Pmod
                            ...
                            for i=1:1:Nwlp
                                ...
                                Gamma_p = sum(gamma_p.(strcat("LP_",pump.modos(p))){i} );
                                v_p = c./lambda_p; Pp0 = P_p0.(strcat("LP_",pump.modos(p)))(i);
                                if m == 1
                                    pmp_xx(m) = pmp_xx(m) + (sigma_abs(lambda_p(i)))*(Pp0*Gamma_p/A_p)/(h*v_p(i));                                   % Término en numerador
                                    pmp_yy(m) = pmp_yy(m) + (sigma_abs(lambda_p(i)) + sigma_ems(lambda_p(i)))*(Pp0*Gamma_p/A_p)/(h*v_p(i));          % Término en denominador
                                else
                                    pmp_xx(m) = (sigma_abs(lambda_p(i)))*(Pp0*Gamma_p/A_p)/(h*v_p(i));                                   % Término en numerador
                                    pmp_yy(m) = (sigma_abs(lambda_p(i)) + sigma_ems(lambda_p(i)))*(Pp0*Gamma_p/A_p)/(h*v_p(i));          % Término en denominador
                                end
                            end
                        end
                        % Potencia de señal
                        for s = 1:1:Smod
                            ...
                            for i = 1:1:Nwl
                                ...
                                Gamma_s = sum(gamma_s.(strcat("LP_",signal.modos(s))){i});
                                v_s = c./lambda_s;  Ps0 = P_s0.(strcat("LP_",signal.modos(s)))(i);
                                if m == 1
                                    sig_xx(m) = sig_xx(m)+(sigma_abs(lambda_s(i))*(Ps0*Gamma_s/A_s)/(h*v_s(i)));                        % Término en numerador
                                    sig_yy(m) = sig_yy(m)+(sigma_abs(lambda_s(i))+sigma_ems(lambda_s(i)))*(Ps0*Gamma_s/A_s)/(h*v_s(i)); % Término en denominador
                                else
                                    sig_xx(m) = (sigma_abs(lambda_s(i))*(Ps0*Gamma_s/A_s)/(h*v_s(i)));                        % Término en numerador
                                    sig_yy(m) = (sigma_abs(lambda_s(i))+sigma_ems(lambda_s(i)))*(Ps0*Gamma_s/A_s)/(h*v_s(i)); % Término en denominador
                                end
                            end
                        end
                        % Potencia ASE
                        for s=1:1:Smod
                            ...
                            for i = 1:1:Nwl
                                ...
                                v_s = c./lambda_s;
                                if m == 1
                                    ase_xx(m) = ase_xx(m)+ sigma_abs(lambda_s(i))*(P_ase0*Gamma_s/A_s)/(h*v_s(i));                           % Término en numerador
                                    ase_yy(m) = ase_yy(m)+(sigma_abs(lambda_s(i)) + sigma_ems(lambda_s(i)) )*(P_ase0*Gamma_s/A_s)/(h*v_s(i)); % Término en denominador
                                else
                                    ase_xx(m) = sigma_abs(lambda_s(i))*(P_ase0*Gamma_s/A_s)/(h*v_s(i));                           % Término en numerador
                                    ase_yy(m) = (sigma_abs(lambda_s(i)) + sigma_ems(lambda_s(i)) )*(P_ase0*Gamma_s/A_s)/(h*v_s(i)); % Término en denominador
                                end
                            end
                        end
                    ...
                    for p = 1:1:Pmod      % Iteracion en cada modo de bombeo
                        ...
                        if(z == 1)
                            for i = 1:1:Nwlp
                                ...
                                Ppp.(strcat("LP_",pump.modos(p)))(i,z) = Pp0 + (sum(N2(z,:))*sigma_ems(lambda_p(i)) - sum(N1(z,:))*sigma_abs(lambda_p(i)))*Gamma_p*Pp0*del_z;
                            end
                            ...
                            for i = 1:1:Nwlp
                                ...
                                Gamma_p = gamma_p.(strcat("LP_",pump.modos(p))){i}(m);
                                %Gamma_p = sum(gamma_p.(strcat("LP_",pump.modos(p))){i});
                                Ppp.(strcat("LP_",pump.modos(p)))(i,z) = Ppp.(strcat("LP_",pump.modos(p)))(i,z-1) + (sum(N2(z,:))*sigma_ems(lambda_p(i)) - sum(N1(z,:))*sigma_abs(lambda_p(i)))*Gamma_p*Ppp.(strcat("LP_",pump.modos(p)))(i,z-1)*del_z;
                            end
                        end
                    end
                    ...
                    for s = 1:1:Smod
                        ...
                        if(z == 1)
                            for i = 1:1:Nwl
                                ...
                                Psp.(strcat("LP_",signal.modos(s)))(i,z) = Ps0 + (sum(N2(z,:))*sigma_ems(lambda_s(i))- sum(N1(z,:))*sigma_abs(lambda_s(i)))*Gamma_s*Ps0*del_z/(1+Ps0/Psat);
                            end
                        else
                            for i = 1:1:Nwl
                                lambda_s = signal.lambda.(strcat("LP_",signal.modos(s)));
                                %Gamma_s = gamma_s.(strcat("LP_",signal.modos(s))){i}(m);
                                Gamma_s = sum(gamma_s.(strcat("LP_",signal.modos(s))){i});
                                Psp.(strcat("LP_",signal.modos(s)))(i,z) = Psp.(strcat("LP_",signal.modos(s)))(i,z-1) + (sum(N2(z,:))*sigma_ems(lambda_s(i))-sum(N1(z,:))*sigma_abs(lambda_s(i)))*Gamma_s*Psp.(strcat("LP_",signal.modos(s)))(i,z-1)*del_z/(1+Psp.(strcat("LP_",signal.modos(s)))(i,z-1)/Psat);
                            end
                        end
                    end
                    ...
                    for s = 1:1:Smod
                        ...
                        if(z == 1)
                            for i = 1:1:Nwl
                                ...
                                Gamma_s = sum(gamma_s.(strcat("LP_",signal.modos(s))){i});
                                v_s = c./lambda_s;
                                Pap.(strcat("LP_",signal.modos(s)))(i,z) = P_ase0 + ((sum(N2(z,:))*sigma_ems(lambda_s(i))-sum(N1(z,:))*sigma_abs(lambda_s(i)))*Gamma_s*P_ase0 + 2*sum(N2(z,:))*sigma_ems(lambda_s(i))*Gamma_s*h*v_s(i)*d_vk)*del_z/(1+P_ase0/Psat);
                            end
                        else
                            for i = 1:1:Nwl
                                lambda_s = signal.lambda.(strcat("LP_",signal.modos(s)));
                                ...
                                Gamma_s = sum(gamma_s.(strcat("LP_",signal.modos(s))){i});
                                v_s = c./lambda_s;
                                Pap.(strcat("LP_",signal.modos(s)))(i,z) = Pap.(strcat("LP_",signal.modos(s)))(i,z-1) + ((sum(N2(z,:))*sigma_ems(lambda_s(i))-sum(N1(z,:))*sigma_abs(lambda_s(i)))*Gamma_s*Pap.(strcat("LP_",signal.modos(s)))(i,z-1) + 2*sum(N2(z,:))*sigma_ems(lambda_s(i))*Gamma_s*h*v_s(i)*d_vk)*del_z/(1+Pap.(strcat("LP_",signal.modos(s)))(i,z-1)/Psat);
                            end
                        end
                    end
                    ...
                    for s = 1:1:Smod
                        ...
                        if(z == 1)
                            for i = 1:1:Nwl
                                ...
                                %Gamma_s = gamma_s.(strcat("LP_",signal.modos(s))){i}(m);
                                Gamma_s = sum(gamma_s.(strcat("LP_",signal.modos(s))){i});
                                Pan.(strcat("LP_",signal.modos(s)))(i,Nz-z+1) = 0+((sum(N2(Nz-z+1,:))*sigma_ems(lambda_s(i))-sum(N1(Nz-z+1,:))*sigma_abs(lambda_s(i)))*Gamma_s*0 + 2*sum(N2(Nz-z+1,:))*sigma_ems(lambda_s(i))*Gamma_s*h*v_s(i)*d_vk)*del_z;
                            end
                        else
                            for i = 1:1:Nwl
                                lambda_s = signal.lambda.(strcat("LP_",signal.modos(s)));
                                ...
                                %Gamma_s = gamma_s.(strcat("LP_",signal.modos(s))){i}(m);
                                Gamma_s = sum(gamma_s.(strcat("LP_",signal.modos(s))){i});
                                Pan.(strcat("LP_",signal.modos(s)))(i,Nz-z+1) = Pan.(strcat("LP_",signal.modos(s)))(i,Nz-z+1+1) + ((sum(N2(Nz-z+1,:))*sigma_ems(lambda_s(i)) - sum(N1(Nz-z+1,:))*sigma_abs(lambda_s(i)))*Gamma_s*Pan.(strcat("LP_",signal.modos(s)))(i,Nz-z+1+1) + 2*sum(N2(Nz-z+1,:))*sigma_ems(lambda_s(i))*Gamma_s*h*v_s(i)*d_vk)*del_z/(1+Pan.(strcat("LP_",signal.modos(s)))(i,Nz-z+1+1)/Psat);                               
                            end
                        end
                    end
                clc
    ...
    for z = 1:Nz
        for s = 1:1:Smod
            ...
            for w = 1:1:Nwl
                ...
                Gamma_s = sum(gamma_s.(strcat("LP_",signal.modos(s))){w});
                
                if(z == 1)
                    for i = 1:1:Nch_ase
                        Pap_sp.(strcat("LP_",signal.modos(s)))(i,z) = P_ase0_sp + ((sum(N2(z,:))*sigma_ems(lambda_ase(i)) - sum(N1(z,:))*sigma_abs(lambda_ase(i)))*Gamma_s*P_ase0_sp + 2*sum(N2(z,:))*sigma_ems(lambda_ase(i))*Gamma_s*h*v_s_sp(i)*d_vk)*del_z/(1+P_ase0_sp/Psat);
                    end
                else
                    for i = 1:1:Nch_ase
                        Pap_sp.(strcat("LP_",signal.modos(s)))(i,z) = Pap_sp.(strcat("LP_",signal.modos(s)))(i,z-1) + ((sum(N2(z,:))*sigma_ems(lambda_ase(i)) - sum(N1(z,:))*sigma_abs(lambda_ase(i)))*Gamma_s*Pap_sp.(strcat("LP_",signal.modos(s)))(i,z-1) + 2*sum(N2(z,:))*sigma_ems(lambda_ase(i))*Gamma_s*h*v_s_sp(i)*d_vk)*del_z/(1+Pap_sp.(strcat("LP_",signal.modos(s)))(i,z-1)/Psat);
                    end
                end
            end
        end
        ...
        for s = 1:1:Smod
            if(z == 1)
                for i = 1:1:Nch_ase
                    Pan_sp.(strcat("LP_",signal.modos(s)))(i,Nz-z+1) = 0+((sum(N2(Nz-z+1,:))*sigma_ems(lambda_ase(i)) - sum(N1(Nz-z+1,:))*sigma_abs(lambda_ase(i)))*Gamma_s*0 + 2*sum(N2(Nz-z+1,:))*sigma_ems(lambda_ase(i))*Gamma_s*h*v_s_sp(i)*d_vk)*del_z;
                end
            else
                for i = 1:1:Nch_ase
                    Pan_sp.(strcat("LP_",signal.modos(s)))(i,Nz-z+1) = Pan_sp.(strcat("LP_",signal.modos(s)))(i,Nz-z+1+1) + ((sum(N2(Nz-z+1,:))*sigma_ems(lambda_ase(i)) - sum(N1(Nz-z+1,:))*sigma_abs(lambda_ase(i)))*Gamma_s*Pan_sp.(strcat("LP_",signal.modos(s)))(i,Nz-z+1+1) + 2*sum(N2(Nz-z+1,:))*sigma_ems(lambda_ase(i))*Gamma_s*h*v_s_sp(i)*d_vk)*del_z/(1+Pan_sp.(strcat("LP_",signal.modos(s)))(i,Nz-z+1+1)/Psat);
                end
            end
        end
