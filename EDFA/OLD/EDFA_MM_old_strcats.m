...
A_p = pi*fibra.radio^2 ;                               % Area efectiva para bombeo
...
for s = 1:1:length(signal.modos) % Mode overlap factor for signal entre modo y perfil de dopaje
    ...
    for i=1:1:Nwl % Cada longitud de onda del modo s
        ...
        %        % Agregar coeficiente de acoplamiento entre bombeo y señal
        %        %CoupCoef = 0;
        %         parfor p = 1:length(pump.modos)
        %             Nwlp = length(pump.lambda.(strcat("LP_",pump.modos(p))));
        %             for pp=1:1:Nwlp
        %                 lambda_p = pump.lambda.(strcat("LP_",pump.modos(p)));
...
for n = 1:1:Sch     % Iteración en nucleos
    ...
    for Q = 1:QQ
        if (Q == 1) % Primera iteracion no calcula ASE en direccion -z
            for z = 1:1:Nz  % Iteraciones a lo largo del EDFA
                ...
                if(z == 1)
                    ...
                    parfor p = 1:1:Pmod
                        ...
                        for i=1:1:Nwlp
                            ...
                            pmp_xx = pmp_xx + (sigma_abs(lambda_p(i)))*(Pp0*Gamma_p/A_p)/(h*v_p(i));                                   % Término en numerador
                            pmp_yy = pmp_yy + (sigma_abs(lambda_p(i)) + sigma_ems(lambda_p(i)))*(Pp0*Gamma_p/A_p)/(h*v_p(i));          % Término en denominador
                        end
                    end
                    % Potencia de señal
                    parfor s = 1:1:Smod
                        ...
                        for i = 1:1:Nwl
                            ...
                            sig_xx = sig_xx+(sigma_abs(lambda_s(i))*(Ps0*Gamma_s/A_s)/(h*v_s(i)));                        % Término en numerador
                            sig_yy = sig_yy+(sigma_abs(lambda_s(i))+sigma_ems(lambda_s(i)))*(Ps0*Gamma_s/A_s)/(h*v_s(i)); % Término en denominador
                    ...
                    parfor s=1:1:Smod
                        ...
                        for i = 1:1:Nwl
                            ...
                            ase_xx = ase_xx+ sigma_abs(lambda_s(i))*(P_ase0*Gamma_s/A_s)/(h*v_s(i));                           % Término en numerador
                            ase_yy = ase_yy+(sigma_abs(lambda_s(i)) + sigma_ems(lambda_s(i)) )*(P_ase0*Gamma_s/A_s)/(h*v_s(i)); % Término en denominador
                ...
                %clc
                %fprintf("Cálculo a lo largo del EDFA: \n")
                %fprintf('%.2f %% \n' ,  (z/Nz)*(1/QQ)* 100 + ((Q-1)/QQ) * 100  ) % Mostrar % de avance del cálculo
                avance = (z/Nz)*(1/QQ) + ((Q-1)/QQ) ;
                waitbar(avance, z_waitbar ,  sprintf("Cálculo a lo largo del EDFA: \n %.2f %%",avance*100 ) )
                set(z_waitbar,'Visible', 'on');
    ...
    for s = 1:1:Smod
        ...
        for i = 1:1:Nwl
            ...
            OSNR.(strcat("LP_",signal.modos(s)))(i,:) = (Psp.(strcat("LP_",signal.modos(s)))(i,:)) - (Pase.(strcat("LP_",signal.modos(s)))(i,:));
...
% Superposición entre señal/bombeo 
