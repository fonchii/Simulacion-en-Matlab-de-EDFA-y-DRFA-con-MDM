        ...
        for p = 1:1:Pmod      % Iteracion en cada modo de bombeo
            ...
            if(z == 1)
                for i = 1:1:Nwlp
                    ...
                    Ppp(i,z,p) = Pp0+(N2(z)*sigma_ems(lambda_p(i)) - N1(z)*sigma_abs(lambda_p(i)))*Gamma_p*Pp0*del_z;
                end
            else
                for i = 1:1:Nwlp
                    lambda_p = pump.lambda.(strcat("LP_",pump.modos(p)));
        ...
        for s = 1:1:Smod
            ...
            if(z == 1)
                for i = 1:1:Nwl
                    ...
                    Psp(i,z,s) = Ps0+(N2(z)*sigma_ems(lambda_s(i))-N1(z)*sigma_abs(lambda_s(i)))*Gamma_s*Ps0*del_z/(1+Ps0/Psat);
                end
            else
                for i = 1:1:Nwl
                    lambda_s = signal.lambda.(strcat("LP_",signal.modos(s)));
