function edfa = EDFA_MMvpi(fibra,signal,pump,ASE)
% Datos de entrada:
% signal (struct)
...
for n = 1:1:Sch     % Iteración en nucleos
        ...
        if (Q == 1) % Primera iteracion no calcula ASE en direccion -z
                ...
                for p = 1:1:Pmod      % Iteracion en cada modo de bombeo
                    ...
                    if(z == 1)
                        ...
                        for i = 1:1:Nwlp
                            lambda_p = pump.lambda.(ModoP(p));
                            Gamma_p = gamma_p.(ModoP(p)){i};
                            Ppp.(ModoP(p))(i,z) = Ppp.(ModoP(p))(i,z-1) + (N2(z-1,:)*sigma_ems(lambda_p(i)) - N1(z-1,:)*sigma_abs(lambda_p(i)))*Gamma_p*Ppp.(ModoP(p))(i,z-1)*del_z/(1+Ppp.(ModoP(p))(i,z-1)/Psat);
                        end
                    end
                end
                ...
                for s = 1:1:Smod
                    ...
                    if(z == 1)
                        for i = 1:1:Nwl
                            ...
                            Gamma_s = gamma_s.(ModoS(s)){i};
                            Ps0 = P_s0.(ModoS(s))(i);
                            %Psat=h*(c/lambda_s(i))*(A_s.(ModoS(s))/Gamma_s)/((sigma_ems(lambda_s(i))+sigma_abs(lambda_s(i)))*tau);
                        ...
                        for i = 1:1:Nwl
                            ...
                            Gamma_s = gamma_s.(ModoS(s)){i};
                            %Psat=h*(c/lambda_s(i))*(A_s.(ModoS(s))/Gamma_s)/((sigma_ems(lambda_s(i))+sigma_abs(lambda_s(i)))*tau);
                            Psp.(ModoS(s))(i,z) = Psp.(ModoS(s))(i,z-1) + (N2(z-1,:)*sigma_ems(lambda_s(i))-N1(z-1,:)*sigma_abs(lambda_s(i)))*Gamma_s*Psp.(ModoS(s))(i,z-1)*del_z/(1+Psp.(ModoS(s))(i,z-1)/Psat);
                        end
                    end
                end
                ...
                for s = 1:1:Smod
                    ...
                    if(z == 1)
                        for i = 1:1:Nwl
                            ...
                            Gamma_s = gamma_s.(ModoS(s)){i};
                            v_s = c./lambda_s;
                            %Psat=h*v_s(i)*(A_s.(ModoS(s))/Gamma_s)/((sigma_ems(lambda_s(i))+sigma_abs(lambda_s(i)))*tau);
            ...
            for z = 1:1:Nz %Iteraciones a lo largo del EDFA
                ...
                for su = 1:1:Smod
                   for sv=1:Smod
                        if (su~=sv)
                            Pan.(ModoS(su))=Pan_aux.(ModoS(su))-h_pccS(su,sv).*(Pan_aux.(ModoS(su))-Pan_aux.(ModoS(sv)))*del_z;
                        end
                   end
                end
                clc
    ...
    for z = 1:Nz
        for s = 1:1:Smod
            P_ase0_sp = P_ase0;
            sum(cell2mat(gamma_s.(ModoS(s))))/length(cell2mat(gamma_s.(ModoS(s))));
            if(z == 1)
                parfor i = 1:1:Nch_ase
        ...
        for su = 1:1:Smod
           for sv=1:Smod
                if (su~=sv)
                    Pan_sp.(ModoS(su))=Pan_spaux.(ModoS(su))-h_pccS(su,sv).*(Pan_spaux.(ModoS(su))-Pan_spaux.(ModoS(sv)))*del_z;
                end
           end
        end
