function edfa = Span_EDFA_GEF_MMvPCCv3(Fibra,Signal,Pump,ASE)
% Datos de entrada:
% signal (struct)
            ...
            for z = 1:1:Nz %Iteraciones a lo largo del EDFA
                ...
                for s = 1:1:Smod
                    ...
                    if(z == 1)
                        parfor i = 1:Nwl
                            Gamma_s = gamma_s.(ModoS(s)){i};
                            PapAux(i) = P_ase0.(ModoS(s))(i); %+ (((N2(z))*sigma_ems(lambda_s(i)) - (N1(z))*sigma_abs(lambda_s(i)))*Gamma_s*P_ase0 + 2*(N2(z))*sigma_ems(lambda_s(i))*Gamma_s*h*v_s(i)*d_vk)*del_z/(1+P_ase0/Psat);
                        end
                        Pap.(ModoS(s))(:,z) = PapAux(:);
                    else
                        parfor i = 1:Nwl
                ...
                for s = 1:1:Smod
                    ...
                    if(z == 1)
                        for i = 1:Nwl
                            ...
                            Pan.(ModoS(s))(i,Nz-z+1) = Pase0+(((N2(Nz-z+1))*sigma_ems(lambda_s(i))-(N1(Nz-z+1))*sigma_abs(lambda_s(i)))*Gamma_s*Pase0 + 2*(N2(Nz-z+1))*sigma_ems(lambda_s(i))*Gamma_s*h*v_s(i)*d_vk)*del_z;
                        end
