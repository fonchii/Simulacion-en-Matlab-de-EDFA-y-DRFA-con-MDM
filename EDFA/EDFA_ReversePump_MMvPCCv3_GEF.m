function edfa = EDFA_ReversePump_MMvPCCv3_GEF(Fibra,Signal,Pump,ASE)
% Datos de entrada:
% signal (struct)
...
for n = 1:1:Sch     % Iteración en nucleos
    ...
    QQ = 5; % Iteraciones para aumentar precisión
    for Q = 1:QQ
        if (Q == 1) % Primera iteracion no calcula ASE en direccion -z

            for z = Nz:-1:1 % Una Iteracion de solo Bombeo en sentido -z
                % Primera Iteración
                sig_xx = 0;
            ...
            for z = 1:1:Nz %Iteraciones a lo largo del EDFA
                ...
                for p = 1:1:Pmod      % Iteracion en cada modo de bombeo
                    ...
                    if(z == 1)
                        ...
                        parfor i = 1:Nwlp
                            Gamma_p = gamma_p.(ModoP(p)){i};
                            PppAux(i) = Ppp.(ModoP(p))(i,Nz-z+2) + ((N2(Nz-z+2))*sigma_ems(lambda_p(i)) - (N1(Nz-z+2))*sigma_abs(lambda_p(i)))*Gamma_p*Ppp.(ModoP(p))(i,Nz-z+2)*del_z;
                        end
