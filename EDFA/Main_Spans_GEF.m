...
Pin = -20; %-35; %[dBm]
Signal.modos = ["01" "11_a" ] ; % "01" "11_a" "21_a"
...
Fibra.SigmaSpectrum = "OptiSystem"; % "VPI"
...
Nspans = 3; %3;
LargoFibra = 90;   % 87,28 -> 5m_100mw  175,73 -> 10m_150mw [km]

alp = load('Dynamic_Attenuation.dat');
Attenuation = @(f) interp1( (alp(:,1).*1e-9) , (alp(:,2)) ,f); % alpha 1555 = 0,1894

% % Grafico Atenuacion
% figure()
% plot(alp(:,1),alp(:,2)); set(gca,'FontSize',8)
...
for span=1:Nspans
   ...
   Signal.P0 = P0_signal; 
    ...
    EDFA = Span_EDFA_GEF_MMvPCCv3_v2(Fibra,Signal,Pump,P0_ASE);
