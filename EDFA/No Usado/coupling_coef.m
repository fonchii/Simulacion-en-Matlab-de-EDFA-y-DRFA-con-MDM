function [ Coupling , paramS , paramP]= coupling_coef(fibra,modos,modop,lambda_s,lambda_p)
% NOTA:
...
if nargin<1
    fibra.radio=25e-6; fibra.n1=1.46; fibra.n2=1.455; modos="01"; modop= "11";
...
paramS = modesparam(modos,lambda_s,fibra) ; paramP = modesparam(modop,lambda_p,fibra) ;
...
if (extract(modos,strlength(modos)))==extract(modos,2)
    if (extract(modop,strlength(modop)))==extract(modop,2)
        ...
        if l_s == 0
            ...
            Ms = @(r,phi) besselj(l_s, (paramS.u * r/ a) ).* ( cos(l_s*phi) )/besselj(l_s,paramS.u) ;
        else 
            %Ms_a = @(r,phi) ( besselj(l_s,vs*r/a) ) .* ( cos(l_s*phi) ) ;
            %Ms_b = @(r,phi) ( besselj(l_s,vs*r/a) ) .* ( sin(l_s*phi) ) ;
            Ms_a = @(r,phi) ( besselj(l_s,paramS.u*r/a) ) .* ( cos(l_s*phi) ) /besselj(l_s,paramS.u) ;
            Ms_b = @(r,phi) ( besselj(l_s,paramS.u*r/a) ) .* ( sin(l_s*phi) ) /besselj(l_s,paramS.u) ;
        end

        if l_p == 0
            %Mp = @(r,phi) ( besselj(l_p,vp*r/a) ) .* ( cos(l_p*phi) ) ;
            Mp = @(r,phi)  besselj(l_p, (paramP.u * r / a) ).* ( cos(l_p*phi) ) / besselj(l_s,paramP.u) ; 
        else 
