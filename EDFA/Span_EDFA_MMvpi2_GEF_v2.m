function edfa = Span_EDFA_MMvpi2_GEF_v2(fibra,signal,pump,ASE)
% Datos de entrada:
% signal (struct)
...
%                 end
                ...
                for s = 1:1:Smod
                    ...
                    if(z == 1)
                        for i = 1:1:Nwl
                            Pap.(ModoS(s))(i,z) = P_ase0_Nulo;%P_ase0.(ModoS(s))(i); % + ((N2(z,:)*sigma_ems(lambda_s(i))-N1(z,:)*sigma_abs(lambda_s(i)))*Gamma_s*P_ase0 + 2*N2(z,:)*sigma_ems(lambda_s(i))*Gamma_s*h*v_s(i)*d_vk)*del_z/(1+P_ase0/Psat);
                        end
                    else
                        for i = 1:1:Nwl
                            lambda_s = signal.lambda.(ModoS(s));
                ...
                for s = 1:1:Smod
                    ...
                    if(z == 1)
                        ...
                        for i = 1:1:Nwl
                            lambda_s = signal.lambda.(ModoS(s));
                            Gamma_s = gamma_s.(ModoS(s)){i};
                            PapAux_amp(i) = Pap.(ModoS(s))(i,z-1) + ((N2(z,:)*sigma_ems(lambda_s(i))-N1(z,:)*sigma_abs(lambda_s(i)))*Gamma_s*Pap.(ModoS(s))(i,z-1) )*del_z; %+ 2*N2(z,:)*sigma_ems(lambda_s(i))*Gamma_s*h*v_s(i)*d_vk)*del_z;
                        end
...
%                 end
    ...
    for s = 1:1:Smod
        Nwl = length(signal.lambda.(ModoS(s)));
        for i = 1:1:Nwl
            OSNR.(ModoS(s))(i,:) = 10*log10(Psp.(ModoS(s))(i,:)/1e-3) - 10*log10(Pap_tot.(ModoS(s))(i,:)/1e-3);
