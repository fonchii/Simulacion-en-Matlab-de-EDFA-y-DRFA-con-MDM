...
for n = 1:1:Sch     % Iteración en nucleos
                ...
                if(z == 1)
                    ...
                    parfor p = 1:1:Pmod
                        ...
                        for i = 1:1:Nwlp
                            ...
                            pmp_xx = pmp_xx + sigma_abs(lambda_p(i))*(Ppp.(ModoP(p))(i,z-1)*Gamma_p/A_p)/(h*v_p(i));                        % Termino en numerador
                            pmp_yy = pmp_yy + (sigma_abs(lambda_p(i)) + sigma_ems(lambda_p(i)))*(Ppp.(ModoP(p))(i,z-1)*Gamma_p/A_p)/(h*v_p(i));           % Termino en denominador
                        end
                    end
                    ...
                    parfor s = 1:1:Smod
                        ...
                        for i = 1:1:Nwl
                            ...
                            sig_xx = sig_xx+(sigma_abs(lambda_s(i))*(Psp.(ModoS(s))(i,z-1)*Gamma_s/A_s)/(h*v_s(i)));
                            sig_yy = sig_yy+(sigma_abs(lambda_s(i)) + sigma_ems(lambda_s(i))) * (Psp.(ModoS(s))(i,z-1)*Gamma_s/A_s)/(h*v_s(i));
                        end
                    end
                    ...
                    parfor s = 1:1:Smod
                        ...
                        for i = 1:1:Nwl
                            ...
                            ase_xx = ase_xx+(sigma_abs(lambda_s(i))*(Pase.(ModoS(s))(i,z-1)*Gamma_s/A_s)/(h*v_s(i)));
                            ase_yy = ase_yy+( sigma_abs(lambda_s(i)) + sigma_ems(lambda_s(i)) )*(Pase.(ModoS(s))(i,z-1)*Gamma_s/A_s)/(h*v_s(i));
                        end
                    end
                end
    ...
    for z = 1:Nz
        for s = 1:1:Smod
            ...
            for w = 1:1:Nwl
                ...
                if(z == 1)
                    for i = 1:1:Nch_ase
                        if i<Nch_ase
                            v_s_sp = c./abs(lambda_ase(i)-lambda_ase(i+1));
                        else
                            v_s_sp = c./abs(lambda_ase(i-1)-lambda_ase(i));
                        end
                        Pap_sp.(ModoS(s))(i,z) = P_ase0_sp + (((N2(z))*sigma_ems(lambda_ase(i)) - (N1(z))*sigma_abs(lambda_ase(i)))*Gamma_s*P_ase0_sp + 2*(N2(z))*sigma_ems(lambda_ase(i))*Gamma_s*h*v_s_sp*d_vk)*del_z/(1+P_ase0_sp/Psat);
                    end
                else
                    for i = 1:1:Nch_ase
                        if i<Nch_ase
                        ...
                        Pap_sp.(ModoS(s))(i,z) = Pap_sp.(ModoS(s))(i,z-1) + (((N2(z))*sigma_ems(lambda_ase(i)) - (N1(z))*sigma_abs(lambda_ase(i)))*Gamma_s*Pap_sp.(ModoS(s))(i,z-1) + 2*(N2(z))*sigma_ems(lambda_ase(i))*Gamma_s*h*v_s_sp*d_vk)*del_z/(1+Pap_sp.(ModoS(s))(i,z-1)/Psat);
                    end
                end
            end
        end
        ...
        for s = 1:1:Smod
            if(z == 1)
                ...
                for i = 1:1:Nch_ase
                    Pan_sp.(strcat("LP_",signal.modos(s)))(i,Nz-z+1) = Pan_sp.(ModoS(s))(i,Nz-z+1+1) + ((sum(N2(Nz-z+1,:))*sigma_ems(lambda_ase(i)) - sum(N1(Nz-z+1,:))*sigma_abs(lambda_ase(i)))*Gamma_s*Pan_sp.(ModoS(s))(i,Nz-z+1+1) + 2*sum(N2(Nz-z+1,:))*sigma_ems(lambda_ase(i))*Gamma_s*h*v_s_sp(i)*d_vk)*del_z/(1+Pan_sp.(ModoS(s))(i,Nz-z+1+1)/Psat);
                end
            end
        end
    ...
    for s = 1:1:Smod
        ...
        for i = 1:1:Nwl
            ...
            OSNR.(strcat("LP_",signal.modos(s)))(i,:) = (Psp.(ModoS(s))(i,:)) - (Pase.(ModoS(s))(i,:));
            gain.(strcat("LP_",signal.modos(s)))(1,i) = 10*log10(Psp.(ModoS(s))(i,end)/Psp.(ModoS(s))(i,1));
        end
    end
