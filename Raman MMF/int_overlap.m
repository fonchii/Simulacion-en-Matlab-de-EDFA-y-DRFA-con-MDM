...
den = ( integral2( @(r,phi) abs( Ii(r,phi).*r ) , 0,a, 0,2*pi ) * ...
    integral2( @(r,phi) abs( Ij(r,phi).*r ) , 0,a, 0,2*pi ) ) + integral2( @(r,phi) abs(Cladi(r,phi).*r) , a,inf , 0,2*pi )*...
    integral2( @(r,phi) abs(Cladj(r,phi).*r) , a,inf , 0,2*pi ) ; 


num = integral2 ( @(r,phi) abs( (Ii(r,phi)).*(Ij(r,phi).*r) ) , 0,a, 0,2*pi ) * pi*fibra.radio^2 ;
