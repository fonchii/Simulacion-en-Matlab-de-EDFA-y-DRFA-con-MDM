    ...
    if sum( Pb.(ModoP{mp})(:,end) ) ~= 0
        for l=1:(length(Z)-1)
            ...
            for wP = 1:length(lambdaP.(ModoP{mp}))
                Pb.(ModoP{mp})(wP,end-l) = Pb.(ModoP{mp})(wP,end)*(exp(-alphaP.(ModoP{mp})*z) );
            end
        end
    end
end
        ...
        for l=1:(length(Z)-1)
            for mp = length(ModoP)
                for wP = 1:length(lambdaP.(ModoP{mp}))
                    ...
                    for ms = 1:length(ModoS)
                        s_sum = sum( fmn.Pump.(ModoP{mp}).(ModoS{ms})(wP,:) * gR.Pump.(ModoP{mp}).(ModoS{ms})(wP,:)'...
                                        .* lambdaS.(ModoS{ms})(:).*Ps.(ModoS{ms})(:,end-l) ) ;
                    end
                    ...
                    for mmp = 1:length(ModoP)
                        if string(ModoP{mp}) ~= string(ModoP{mmp}) 
                            for wwP = 1:length(lambdaP.(ModoP{mmp}))
                                if lambdaP.(ModoP{mmp})(wwP) == lambdaP.(ModoP{mp})(wP)
                                    eta_sum = eta_sum + eta_fun(lambdaP.(ModoP{mp})(wP)) * ( Pf.(ModoP{mmp})(wwP,end-l) + Pb.(ModoP{mmp})(wwP,end-l) );
                                end
                            end
                        end
                    end

                    Pb.(ModoP{mp})(wP,end-l) = Pb.(ModoP{mp})(wP,end-l) + deltaZ*( -alphaP.(ModoP{mp})*Pb.(ModoP{mp})(wP,end-l) - (1/lambdaP.(ModoP{mp})(wP))*s_sum*Pb.(ModoP{mp})(wP,end-l) )...
                                            - eta_sum; 
                end
            end
        end
        
        % Volver a calcular señal y potencia Forward
        for l = 1:(length(Z)-1)
            % Señal
            for ms = 1:length(ModoS)
                for wS = 1:length(lambdaS.(ModoS{ms}))
