...
% BOMBEOS : 
    % LP01
 In.Pump.LP01.Wavelengths       = [1430 1470 ] ;                                                        % [nm]
 In.Pump.LP01.Powers            = [50 80]*1e-3;             % [mW]
 In.Pump.LP01.Alpha             = [0.29]; 
    % LP11
In.Pump.LP11.Wavelengths        = [1430 1470 ] ;                                                         % [nm]
In.Pump.LP11.Powers             = [50 80]*1e-3;                                                     % [mW]
In.Pump.LP11.Alpha              = [0.29];                                                           % [dB/km]
%     % LP21
% In.Pump.LP21.Wavelengths        = [1430 1470 ] ;                                                         % [nm]
% In.Pump.LP21.Powers             = 50 *1e-3*ones( 1,length(In.Pump.LP01.Wavelengths) );                                                     % [mW]
% In.Pump.LP21.Alpha              = [0.29];                                                           % [dB/km]
%     % LP02
% In.Pump.LP02.Wavelengths        = [1430 1470 ] ;                                                         % [nm]
% In.Pump.LP02.Powers             = 50.*1e-3*ones( 1,length(In.Pump.LP01.Wavelengths) );                                                     % [mW]
...
for ms = 1:length(Raman.ModoS)% Potencia Señal
    ...
    xlabel("Largo [km]") ; title(strcat("Signal Modo ", Raman.ModoS{ms})) ; legend(strlambda,'Location','best')
    fs = fs+1;
end

% for ms = 1:length(Raman.ModoS)% Potencia Señal Off
%     figure(1); 
%     strlambda = strcat( num2str( In.Signal.(Raman.ModoS{ms}).Wavelengths(:)) , "nm OFF");
%     subplot(1,length(Raman.ModoS),fs) ; plot(z,10.*log10(signal.Off.(Raman.ModoS{ms})./1e-3) ) ,ylabel("Potencia [dBm]") ; hold on
