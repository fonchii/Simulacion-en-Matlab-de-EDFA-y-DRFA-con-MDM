function  = coupling_coef(fibra,modos,modop,lambda_s,lambda_p)
% NOTA:
...
if (extract(modos,strlength(modos)))==extract(modos,2)
    if (extract(modop,strlength(modop)))==extract(modop,2)
        ...
        if l_s==0 
            ...
            %num = integral2( @(r,phi) abs( Ms_b(r,phi) + Ms_a(r,phi) ).* abs( Mp(r,phi) ) , 0, a, 0, 2*pi).^2 ;
            num = integral2( @(r,phi) abs( Ms_b(r,phi) ).* abs( Mp(r,phi) ) , 0, a, 0, 2*pi).^2 + integral2( @(r,phi) abs( Ms_a(r,phi) ).* abs( Mp(r,phi) ) , 0, a, 0, 2*pi).^2 ;
            %den = integral2( @(r,phi) abs( Ms_b(r,phi) + Ms_a(r,phi) ).^2 , 0, a, 0, 2*pi) * integral2( @(r,phi) ( abs( Mp(r,phi) ) ).^2 , 0, a, 0, 2*pi);
            den = integral2( @(r,phi) abs( Ms_b(r,phi) ).^2 , 0, a, 0, 2*pi) * integral2( @(r,phi) ( abs( Mp(r,phi) ) ).^2 , 0, a, 0, 2*pi) + ...
                integral2( @(r,phi) abs( Ms_a(r,phi) ).^2 , 0, a, 0, 2*pi) * integral2( @(r,phi) ( abs( Mp(r,phi) ) ).^2 , 0, a, 0, 2*pi);
        end
    end
    ...
    if (extract(modop,strlength(modop)))==extract(modop,2)
        ...
        if l_p==0
                ...
                %num =  integral2( @(r,phi) abs( Ms(r,phi)).* abs( Mp_b(r,phi) + Mp_a(r,phi) ) , 0, a, 0, 2*pi).^2 ;
                num =  integral2( @(r,phi) abs( Ms(r,phi)).* abs( Mp_a(r,phi) ) , 0, a, 0, 2*pi).^2 + integral2( @(r,phi) abs( Ms(r,phi)).* abs( Mp_b(r,phi) ) , 0, a, 0, 2*pi).^2 ;
                den = integral2( @(r,phi) abs( Ms(r,phi) ).^2 , 0, a, 0, 2*pi) * integral2( @(r,phi) ( abs( Mp_b(r,phi) + Mp_a(r,phi) ) ).^2 , 0, a, 0, 2*pi);
        end
    else
